...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for sentence in source_text.split("\n"):
        source_id.append([source_vocab_to_int[words] for words in sentence.split()])
    
    for sentence in target_text.split("\n"):
        target_id.append([target_vocab_to_int[words] for words in sentence.split()] +                          [target_vocab_to_int['<EOS>']])
...
def model_inputs():
    ...
    """
    # TODO: Implement Function
    input_text = tf.placeholder(tf.int32, [None, None], "input")
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    return tf.concat([tf.fill([batch_size, 1], target_vocab_to_int['<GO>']),                       tf.strided_slice(target_data, [0, 0], [batch_size, -1], [1, 1])],                       axis = 1)
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    """
    # TODO: Implement Function
    cells = [tf.contrib.rnn.LSTMCell(rnn_size) for i in range(num_layers)]
    encoder = tf.contrib.rnn.MultiRNNCell(cells)
    
    outputs, result = tf.nn.dynamic_rnn(encoder, rnn_inputs, dtype = tf.float32)
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    results, state, context = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell, decoder_fn,                                                                      dec_embed_input, sequence_length,                                                                      scope = decoding_scope)
    train_logits = output_fn(results)
    return train_logits
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    """
    # TODO: Implement Function
    infer_fn = tf.contrib.seq2seq.simple_decoder_fn_inference(output_fn, encoder_state, dec_embeddings,                                                               start_of_sequence_id, end_of_sequence_id,                                                               maximum_length - 1, vocab_size)
    
    drop = tf.contrib.rnn.DropoutWrapper(dec_cell, keep_prob)
    inference_logits, state, context = tf.contrib.seq2seq.dynamic_rnn_decoder(drop, infer_fn,                                                                               scope = decoding_scope                                                                              )
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    """
    # TODO: Implement Function
    start_sentence = target_vocab_to_int['<GO>']
    end_sentence = target_vocab_to_int['<EOS>']
    
    dropout = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(rnn_size), keep_prob)
    ...
    with tf.variable_scope("decoding", reuse = True) as decoding_scope:
        inference_logits = decoding_layer_infer(encoder_state, cell, dec_embeddings,                                                 start_sentence, end_sentence, sequence_length,                                                 vocab_size, decoding_scope, output_fn, keep_prob)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    enc_encode = encoding_layer(enc_embed, rnn_size, num_layers, keep_prob)
    
    dec = process_decoding_input(target_data, target_vocab_to_int, batch_size)
    dec_embeddings = tf.Variable(tf.random_uniform([target_vocab_size, dec_embedding_size]))
    dec_embed = tf.nn.embedding_lookup(dec_embeddings, dec)
    
    train_logits, inference_logits = decoding_layer(dec_embed, dec_embeddings, enc_encode,                                                     target_vocab_size, sequence_length, rnn_size,                                                     num_layers, target_vocab_to_int, keep_prob)
